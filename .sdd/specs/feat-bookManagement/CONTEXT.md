...
- **Inventory Metrics:** `totalQuantity` là số BookCopy chưa bị loại khỏi kho; `availableQuantity` là số suất còn có thể cấp, bằng năng lực bản sao vật lý đủ điều kiện sau khi trừ các Reservation `readypickup`. Đây không phải phép đếm trực tiếp mọi BookCopy `available`.
- **Reservation Allocation:** Reservation `pending/readypickup` chỉ giữ suất của đầu sách và luôn có `bookCopyId=NULL`; Barcode cụ thể chỉ được Thủ thư chọn và gán khi checkout.
...
- **Capacity First:** Khi thêm hoặc import BookCopy cho đầu sách đang lưu thông, hệ thống phải ưu tiên đôn người đầu hàng `pending` thành `readypickup`; chỉ tăng `availableQuantity` nếu không có người chờ. Email chỉ được xếp hàng sau khi transaction commit.
- **Normalized Category:** Tên Category được trim và duy nhất không phân biệt hoa/thường ở Database; request đồng thời phải nhận lỗi nghiệp vụ thân thiện.
