...
- **FR-F6-01:** WHEN Thủ thư quét mã định danh, THE system SHALL truy vấn `UserLockReason`. WHERE tồn tại `reason` == 'unpaid', THE system SHALL từ chối giao dịch và báo lỗi: "Tài khoản đang nợ phạt" (BR-22).
...
- **FR-F6-04:** WHEN Thủ thư nhận trả sách với Condition IN ('damaged', 'lost'), THE system SHALL thực thi Transaction: UPDATE `Book.totalQuantity` = `totalQuantity` - 1, UPDATE `BookCopy.status` = 'unavailable', INSERT `Fine`, ĐỒNG THỜI INSERT `UserLockReason` ('unpaid') VÀ UPDATE `[User].status` = 'locked' (BR-24).
...
- **FR-F6-07:** WHEN Thủ thư xác nhận duyệt đơn `Payment` tiền mặt, THE system SHALL thực thi Transaction: UPDATE `Payment.status` = 'completed', UPDATE `Fine.status` = 'paid', VÀ DELETE bản ghi có `reason` = 'unpaid' trong `UserLockReason`.
- **FR-F6-08:** WHILE duyệt thanh toán hoàn tất, THE system SHALL đếm số bản ghi còn lại trong `UserLockReason` của User. WHERE COUNT == 0, THE system SHALL UPDATE `[User].status` = 'active'. WHERE COUNT > 0, THE system SHALL giữ nguyên trạng thái khóa (BR-25).
...
- [ ] Mượn sách khi tài khoản nợ phạt (UserLockReason='unpaid'): Bị chặn.
...
- [ ] Trả sách 'damaged/lost': Sách chuyển thành 'unavailable', totalQuantity giảm 1, user bị khóa lập tức.
- [ ] Thanh toán phạt: Gỡ cờ 'unpaid'. User có cờ 'adminban' VẪN BỊ KHÓA sau khi thanh toán.

## 8. Out of Scope
- KHÔNG thanh toán qua cổng VNPAY/Ngân hàng điện tử.
