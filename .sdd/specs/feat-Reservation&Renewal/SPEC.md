...
* **BR-36 (Reservation Pickup Limit):** Đơn đặt trước ở trạng thái 'readypickup' giữ một suất khả dụng của đầu sách, không giữ một BookCopy cụ thể (`bookCopyId=NULL`), trong khoảng thời gian từ cấu hình RESERVATION_HOLD_DAYS. Nếu quá hạn, suất được chuyển cho người chờ tiếp theo; chỉ khi không còn người chờ mới cộng lại `Book.availableQuantity`.
...
* **FR-67 (Dọn dẹp đơn đặt trước quá hạn theo cơ chế Lazy Load):** WHEN các Controller/Service liên quan (LibrarianDashboardServlet, DeskDashboardServlet, CheckInServlet, CheckOutServlet, BookDetailServlet, OnlineCirculationService.reserveBook) được truy cập hoặc thực thi, THE system SHALL tự động gọi ReservationExpirationProcessor.processExpiration() [Lazy Load]: (1) Lấy RESERVATION_HOLD_DAYS từ SystemConfig (default 3 ngày), (2) Truy vấn ReservationDAO.findExpiredReadyPickup(NOW()) để lấy tất cả Reservation WHERE status='readypickup' AND endDate < NOW(), (3) Ghi log số lượng đơn hết hạn tìm được, (4) Với mỗi expired reservation: gọi FR-68 để xử lý cancel + đôn hàng chờ trong transaction riêng biệt.
  * *Mapping:* UC-43 / BR-36
* **FR-68 (Hủy đặt trước và đôn hàng chờ tự động):** For each expired Reservation found trong FR-67, THE system SHALL mở DB Transaction: (1) UPDATE Reservation SET status='cancelled', queuePosition=NULL, bookCopyId=NULL, endDate=NOW() WHERE reservationId=?, (2) khóa đầu sách và kiểm tra hàng chờ, (3) **Có người chờ tiếp theo**: UPDATE người tiếp theo SET queuePosition=0, status='readypickup', bookCopyId=NULL, endDate=NOW()+RESERVATION_HOLD_DAYS và giữ nguyên `Book.availableQuantity`, (4) **Không có người chờ**: tăng `Book.availableQuantity` đúng 1 nếu đầu sách còn lưu thông, (5) INSERT AuditLog và commit, (6) gửi email ngoài transaction.
  * *Mapping:* UC-43 / BR-36
...
### Bảng `Reservation`
* `reservationId` (INT, PK), `userId` (FK), `bookId` (FK), `bookCopyId` (FK, NULL ở pending/readypickup và chỉ có giá trị sau checkout), `status` (pending/readypickup/fulfilled/cancelled), `queuePosition` (INT), `startDate`, `endDate`
