...
- [ ] **VNPAY Sandbox Integration**
  ...
  - [ ] Viết `PaymentServlet.java` tạo URL thanh toán chuyển hướng sang VNPAY Sandbox.
  - [ ] Viết `PaymentCallbackServlet.java` để hứng kết quả trả về, thực hiện kiểm tra chữ ký Secure Hash, cập nhật trạng thái hóa đơn phạt và mở khóa tài khoản người dùng tương ứng.

- [ ] **System Configuration & Notifications**
  - [ ] Viết `SystemConfigurationsDAO.java` để đọc/ghi các thông số cấu hình chính sách.
  - [ ] Xây dựng Servlet và trang JSP `manage-configs.jsp` cho quản lý sửa thông số chính sách thư viện.
...
- [ ] **Admin Audit Trail & Account Management**
  - [ ] Viết API/Servlet `AuditLogServlet.java` truy vấn và lọc dữ liệu `AuditLogs`.
  - [ ] Thiết kế giao diện lọc nhật ký nâng cao `view-audit-logs.jsp` cho Admin.
  - [ ] Triển khai tính năng khóa/mở khóa tài khoản người dùng tại giao diện `manage-users.jsp` (Cập nhật User status, tuyệt đối không được xóa).
