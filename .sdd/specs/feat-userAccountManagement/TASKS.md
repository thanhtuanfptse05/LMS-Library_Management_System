...
| **T-UAM-05** | View/List/Update Servlets | `UserListServlet.java`<br>`UserDetailServlet.java` | 2h | T-04 | GET render danh sách. POST update thông tin và đổi `status` thành `locked` nếu có yêu cầu thông qua `UserService` (tự động kích hoạt Audit Log). |
