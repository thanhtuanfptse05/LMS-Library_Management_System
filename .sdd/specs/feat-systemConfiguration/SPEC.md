...
Cho phép Quản trị viên (Admin) và Admin linh hoạt thay đổi các thông số cấu hình chính sách của thư viện (mức phạt trễ hạn, hạn mức mượn sách, số ngày mượn mặc định cho từng đối tượng, số lần gia hạn tối đa, thời gian giữ chỗ đặt trước) mà không cần can thiệp mã nguồn hay khởi động lại hệ thống.

## 2. Actors & Roles (Tác nhân & Quyền hạn)
* **Quản trị viên (Admin) / Admin:** Xem và điều chỉnh các tham số cấu hình chính sách hệ thống.

## 2.5 Use Cases (Danh sách Use Cases đúng theo registry)
* **UC-32 (View System Configuration):** Actor: Admin | (Xem cấu hình hệ thống): Quản trị viên hoặc quản trị viên xem các thông số vận hành của hệ thống như chính sách mượn sách, tiền phạt, giới hạn.
* **UC-33 (Update System Configuration):** Actor: Admin | (Cập nhật cấu hình hệ thống): Thay đổi các thông số vận hành thông qua giao diện quản trị mà không cần can thiệp mã nguồn.
...
* **FR-84 (Xem cấu hình nhóm library của Admin):** WHEN AdminSystemConfigServlet.doGet() được gọi bởi ADMIN, THE system SHALL truy vấn SystemConfigDAO.findByGroup(conn, "library") để lấy danh sách các cấu hình chính sách mượn/trả và đặt trước, sau đó forward sang admin/system-config-list.jsp.
...
* **FR-85 (Xem toàn bộ cấu hình của Admin):** WHEN AdminSystemConfigServlet.doGet() được gọi bởi ADMIN, THE system SHALL lấy toàn bộ cấu hình hệ thống từ DB, hỗ trợ lọc theo nhóm thông qua query parameter 'group', sau đó forward sang admin/system-config-list.jsp.
  * *Mapping:* UC-32 / BR-31, BR-40
* **FR-86 (Cập nhật cấu hình hệ thống):** WHEN AdminSystemConfigServlet hoặc AdminSystemConfigServlet nhận POST cập nhật, THE system SHALL gọi SystemConfigService.update(key, value, actorId, role) để: (1) Kiểm tra key có trong whitelist (KEY_TYPES), (2) Xác thực giá trị hợp lệ theo kiểu dữ liệu quy định, (3) Chặn Admin nếu cố ý sửa các key nhóm khác (ngoại trừ sepay), (4) Thực thi UPDATE câu lệnh SQL, ghi Audit Log và reload cache config.
...
* **FR-88 (Bảo mật chặn xóa cấu hình):** WHEN AdminSystemConfigServlet nhận POST xóa cấu hình (action=delete), THE system SHALL ném ra lỗi ValidationException "Cấm tuyệt đối việc xóa cấu hình khỏi hệ thống" và chặn đứng mọi hoạt động xóa dữ liệu.
  * *Mapping:* UC-33 / BR-30
* **FR-89 (Hiển thị màu badge nhóm cấu hình):** WHEN kết xuất danh sách cấu hình trên JSP, THE system SHALL render class CSS tương ứng để tạo badge màu: library=green, fine=yellow, notification=blue, system=gray.
  * *Mapping:* UC-32 / BR-31
* **FR-90 (Phân quyền AuthFilter cấu hình Admin):** WHERE người dùng không có vai trò ADMIN cố tình truy cập `/admin/system-config`, THE system SHALL chặn lại tại AuthFilter và trả về lỗi SC_FORBIDDEN (403).
  * *Mapping:* UC-32 / BR-31
* **FR-91 (Phân quyền AuthFilter cấu hình Admin):** WHERE người dùng không có vai trò ADMIN cố tình truy cập `/admin/system-config`, THE system SHALL chặn lại tại AuthFilter và trả về lỗi SC_FORBIDDEN (403).
  * *Mapping:* UC-32 / BR-31
* **FR-92 (Chặn sửa đổi chéo nhóm config):** WHERE Admin cố ý chỉnh sửa key cấu hình thuộc nhóm system/fine/notification thông qua giả lập request, THE system SHALL phát hiện tại Service layer và trả về lỗi "Bạn không có quyền chỉnh sửa nhóm cấu hình này."
  * *Mapping:* UC-33 / BR-31
...
* **Bảo mật:** Phân quyền nghiêm ngặt chỉ Admin mới có quyền sửa đổi cấu hình.
...
- [ ] [TC-CONF-01] Admin cập nhật giá trị mức phạt theo ngày thành công và áp dụng ngay cho phiếu mượn trả sau đó.
