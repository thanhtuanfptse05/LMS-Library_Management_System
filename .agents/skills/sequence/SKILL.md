---
name: sequence
description: Tạo sequence diagram Mermaid cho flow nghiệp vụ BA (login, checkout, webhook, error recovery), xuất vào docs/{feature}/srs/{feature}-flows.md. Xác nhận kế hoạch trước khi ghi file và kiểm tra cú pháp Mermaid qua mmdc.
---
...
Kích hoạt bằng câu lệnh tự nhiên hoặc cú pháp lệnh: `/sequence`
...
9.7. **Self-Review Gate (CHỈ khi vượt ngưỡng phức tạp)** — nếu fact-list ở bước 2.5 có **≥3 Alternative/Error Flow**, HOẶC diagram có **≥4 participant**, HOẶC **nesting alt/opt ≥2 tầng**, HOẶC có **callback/timeout/webhook**, thực hiện tự kiểm tra (Self-Review) kỹ thuật theo **Tiêu chí tự review diagram** ở dưới trước khi báo hoàn tất. Đo theo **tổng độ phức tạp** — số actor đơn thuần không đủ. Dưới mọi ngưỡng trên, bước 9.6 tự-đối-chiếu đã đủ — SKIP 9.7, đi thẳng bước 10 để tránh overhead cho case đơn giản.
   - Kiểm tra kỹ các mục: Actor completeness, Branch/alt-flow completeness, Message order sanity, Orphan branch.
   - Phát hiện lỗi BLOCKING (thiếu actor, thiếu nhánh alt/error flow có ID trong fact-list) → tự bổ sung vào section, verify lại 9.5+9.6, rồi tiếp bước 10.
...
1. **Actor/participant completeness:** Mỗi actor trong fact-list có xuất hiện làm `participant`/`actor` trong mermaid code không?
2. **Branch/alt-flow completeness:** Mỗi Alternative/Error Flow (ID A1/A1.1...) trong fact-list có xuất hiện thành 1 `alt`/`opt` block với đủ label/điều kiện không?
3. **Message order sanity:** Thứ tự message có khớp logic mô tả không (vd response không được đến trước request tương ứng)?
4. **Orphan branch:** Có nhánh nào trong mermaid code KHÔNG khớp bất kỳ fact nào trong fact-list (bịa thêm case không có nguồn)? Flag để user xác nhận.
5. **Severity rubric:**
   - **BLOCKING:** Actor có trong fact-list nhưng vắng mặt trong code; Alternative/Error Flow trong fact-list nhưng không có `alt`/`opt` block tương ứng.
   - **WARNING:** Message order nghi ngờ sai logic; label mơ hồ; orphan branch.
