---
name: activity
description: Tạo activity diagram Mermaid cho quy trình nghiệp vụ BA có điều kiện/rẽ nhánh phức tạp, xuất vào docs/{feature}/srs/{feature}-flows.md. Xác nhận kế hoạch trước khi ghi file và kiểm tra cú pháp qua mmdc.
---
...
Kích hoạt bằng câu lệnh tự nhiên hoặc cú pháp lệnh: `/activity`
...
- **Per .agents/rules/diagram-selection.md** — check process có ≥3 decisions hoặc có parallel; nếu đơn giản tuyến tính → suggest `/sequence`.
...
9.7. **Self-Review Gate (CHỈ khi vượt ngưỡng phức tạp)** — thực hiện tự kiểm tra (Self-Review) kỹ thuật theo **Tiêu chí tự review diagram** ở dưới trước khi báo hoàn tất, khi vượt bất kỳ ngưỡng nào (đo theo **tổng độ phức tạp**): **≥3 lane** HOẶC **≥5 decision point** HOẶC **nesting decision ≥2 tầng** HOẶC có **loop/retry quay lại**. Dưới mọi ngưỡng trên, bước 9.6 tự-đối-chiếu đã đủ — SKIP 9.7, đi thẳng bước 10.
   - Kiểm tra kỹ các mục: Lane completeness, Decision coverage, Dead-end detection, Gateway branch count.
   - Phát hiện lỗi BLOCKING (thiếu lane, thiếu decision point, dead-end, gateway chỉ có 1 nhánh) → tự bổ sung vào section, verify lại 9.5+9.6, rồi tiếp bước 10.
   - Loop tối đa 2 vòng — vòng 2 vẫn lỗi → báo user rõ findings còn tồn đọng, để user quyết định trước khi báo report.
...
## Tiêu chí tự review diagram (Self-Review)

Khi diagram vượt ngưỡng phức tạp (bước 9.7), agent bắt buộc tự review theo các tiêu chí chuyên sâu sau trước khi báo hoàn tất cho user:
1. **Lane completeness:** Mỗi lane đã xác nhận ở bước 3.5 có xuất hiện thành 1 `subgraph` không?
2. **Decision coverage:** Mỗi decision point trong fact-list có xuất hiện thành 1 diamond với đủ nhánh (yes/no) không?
3. **Dead-end / loose-end detection:** Mọi node có ít nhất 1 outgoing edge dẫn tới 1 end node (`((End))` hoặc tương đương) — không có nhánh cụt giữa chừng.
4. **Gateway branch count:** Mỗi diamond (`{...}`) có ≥2 outgoing edge với label rõ ràng không — tránh decision point chỉ có 1 nhánh (vô nghĩa) hoặc không có label.
5. **Severity rubric:**
   - **BLOCKING:** Thiếu lane; thiếu decision point; dead-end (1 path không dẫn tới end node); gateway chỉ có 1 outgoing edge.
   - **WARNING:** Nhánh có label mơ hồ ("nếu lỗi" không nói lỗi gì cụ thể); orphan branch (code có nhánh không khớp fact nào trong fact-list).
...
- .agents/rules/diagram-selection.md
- .agents/rules/feature-bootstrap.md
- @../../../_templates/diagram-activity.md
- @../../scripts/mermaid-verify.mjs (render-verify sau Write — bước 9.5)
- Tiêu chí tự review diagram (Self-Review inline) (Diagram_Reviewer — review coverage khi vượt ngưỡng phức tạp, bước 9.7)
