...
CREATE TABLE InventorySession (
    ...
    CONSTRAINT CK_InventorySession_Status
        CHECK ([status] IN ('draft', 'counting', 'reviewing', 'completed', 'cancelled'))
);
...
CREATE TABLE InventoryItem (
    ...
    CONSTRAINT CK_InventoryItem_Result
        CHECK (result IN ('pending', 'matched', 'missing', 'misplaced'))
);

CREATE UNIQUE INDEX UX_InventoryItem_Session_Copy
    ON InventoryItem(inventorySessionId, bookCopyId);
