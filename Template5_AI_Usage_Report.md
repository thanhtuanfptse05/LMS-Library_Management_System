# BÁO CÁO SỬ DỤNG CÔNG CỤ TRÍ TUỆ NHÂN TẠO (AI USAGE REPORT - TEMPLATE 5)

## 1. THÔNG TIN CHUNG
- **Môn học:** SWP391 - Software Project
- **Tên dự án:** LMS - Library Management System (Hệ thống Quản lý Thư viện)
- **Công cụ AI được sử dụng chính:** Gemini Advanced / Antigravity IDE
- **Mục đích sử dụng AI:** Hỗ trợ phân tích thiết kế (UML, Database), sinh mã nguồn (Code generation), tìm và sửa lỗi (Debugging), tự động hóa kiểm thử (Unit Test generation), và tinh chỉnh giao diện (UI/UX CSS).

## 2. PHÂN CÔNG VÀ TRỌNG TÂM ÁP DỤNG AI THEO THÀNH VIÊN
Dựa trên các chức năng (Features) của hệ thống:
- **Cao Thanh Tuấn:** 
  - *Tính năng:* F1 (Authentication), F2 (Profile), F7 (Notification), F9 (Fine & Payment), F15 (Librarian Dashboard), F18 (Public Pages).
  - *Trọng tâm AI:* Code xử lý luồng đăng nhập SSO, OTP, tích hợp Webhook SePay, tối ưu hóa giao diện.
- **Vũ Văn Quyết:** 
  - *Tính năng:* F3 (User Account Management), F10 (System Config), F11 (System Reports), F12 (Audit Logs), F15 (KPI).
  - *Trọng tâm AI:* Sinh mã phân trang, Export Excel báo cáo (Apache POI), tối ưu truy vấn logs.
- **Nguyễn Huy Chương:** 
  - *Tính năng:* F4 (Book Management), F13 (Book Maintenance).
  - *Trọng tâm AI:* Xử lý upload ảnh (Multipart), logic tự động sinh mã vạch (Barcode), quản lý tồn kho.
- **Lê Thế Bảo:** 
  - *Tính năng:* F5 (Online Reservation), F8 (Book Discovery), F19 (Async Email), F20 (Book Suggestion).
  - *Trọng tâm AI:* Viết các truy vấn SQL phức tạp cho Recommendation, kiến trúc Queue gửi mail bất đồng bộ, xử lý logic Transaction luân chuyển sách.
- **Vũ Doanh Thái:** 
  - *Tính năng:* F6 (Desk Circulation), F14 (Chat bot).
  - *Trọng tâm AI:* Logic xử lý mượn/trả sách tại quầy, Intent Classification (phân loại ý định) cho Chatbot.

## 3. TỔNG HỢP NHẬT KÝ TƯƠNG TÁC (AI LOG)
*Chi tiết các lượt tương tác (prompts/responses), các lỗi đã phát hiện và quy trình khắc phục được tổng hợp tự động từ nhật ký làm việc của nhóm:*
