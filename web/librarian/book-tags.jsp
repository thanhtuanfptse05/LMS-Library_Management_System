...
<div class="d-flex main-wrapper overflow-hidden">
    <main class="flex-grow-1 overflow-y-auto main-content-layout">
        ...
        <div class="container-fluid px-4 py-4 bm-page">
            ...
            <form class="bm-filter-card bm-filter-card--compact mb-2 bm-auto-filter" method="get" action="${pageContext.request.contextPath}/book-management/tags">
            ...
            <section class="bm-table-card bm-table-card--primary"><div class="table-responsive"><table class="table table-lms">
                <thead><tr><th>Tên tag</th><th>Số đầu sách</th><th>Trạng thái</th><th>Cập nhật gần nhất</th><th>Người cập nhật</th><th class="bm-action-column"><span class="visually-hidden">Thao tác</span></th></tr></thead>
                <tbody>
                    <c:forEach var="tag" items="${tags}"><tr><td><span class="bm-tag ${tag.status == 'hidden' ? 'bm-tag--secondary' : ''}"><c:out value="${tag.name}" /></span></td><td><a class="bm-count-link" href="${pageContext.request.contextPath}/book-management/titles?tagId=${tag.tagId}">Xem <fmt:formatNumber value="${tag.bookCount}" /> đầu sách</a></td><td><span class="bm-badge ${tag.status == 'active' ? 'bm-badge--success' : 'bm-badge--neutral'}">${tag.status == 'active' ? 'Đang dùng' : 'Đã ẩn'}</span></td><td><fmt:formatDate value="${tag.updatedAt}" pattern="dd/MM/yyyy HH:mm" /></td><td><c:out value="${tag.updatedByName}" /></td><td class="bm-action-column"><c:choose><c:when test="${canEdit}"><a class="bm-action-icon" href="${pageContext.request.contextPath}/book-management/tags?editId=${tag.tagId}" title="Chỉnh sửa tag sách" aria-label="Chỉnh sửa tag sách"><span class="material-symbols-outlined" aria-hidden="true">edit</span></a></c:when><c:otherwise><span class="bm-badge bm-badge--neutral">Chỉ xem</span></c:otherwise></c:choose></td></tr></c:forEach>
                    <c:if test="${empty tags}"><tr><td colspan="6"><div class="bm-empty-state"><span class="material-symbols-outlined">sell</span><strong>Không tìm thấy tag sách</strong><span>Hãy thử thay đổi từ khóa hoặc trạng thái.</span></div></td></tr></c:if>
                </tbody>
            </table></div></section>
        </div>
