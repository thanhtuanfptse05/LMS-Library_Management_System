            ...
            <section class="bm-table-card"><div class="table-responsive"><table class="table table-lms">
                ...
                <tbody>
                    <c:forEach var="copy" items="${copies}"><tr>
                        ...
                        <td><c:choose><c:when test="${canEdit and copy.condition != 'good'}"><a class="btn btn-sm bm-incident-button" href="${pageContext.request.contextPath}/librarian/book-damaged-lost.jsp">Xử lý sự cố</a></c:when><c:when test="${canEdit and copy.status == 'available'}"><a class="btn btn-sm bm-btn-secondary" href="${pageContext.request.contextPath}/book-management/copies?editId=${copy.bookCopyId}">Cập nhật</a></c:when><c:otherwise><span class="bm-badge bm-badge--neutral">Chỉ xem</span></c:otherwise></c:choose></td>
                    </tr></c:forEach>
...
<c:if test="${canEdit and not empty editCopy}">
    <div class="modal fade bm-modal" id="editCopyModal" tabindex="-1" aria-hidden="true" data-auto-open="true"><div class="modal-dialog"><div class="modal-content"><form method="post" action="${pageContext.request.contextPath}/book-management/copies"><input type="hidden" name="action" value="update"><input type="hidden" name="bookCopyId" value="${editCopy.bookCopyId}"><div class="modal-header"><div><h5 class="modal-title">Cập nhật bản sao</h5><p class="bm-section-note mb-0">Mã vạch <strong><c:out value="${editCopy.barcode}" /></strong> không thể thay đổi.</p></div><button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Đóng"></button></div><div class="modal-body"><div class="mb-3"><label class="form-label">Vị trí</label><input class="form-control" name="location" required maxlength="255" list="locationOptions" value="<c:out value="${editCopy.location}" />"></div><div><label class="form-label">Tình trạng</label><select class="form-select" name="condition"><option value="good" ${editCopy.condition == 'good' ? 'selected' : ''}>Tốt</option><option value="damaged">Hỏng</option><option value="lost">Mất</option></select><p class="bm-section-note mt-2 mb-0">Chuyển sang Hỏng hoặc Mất sẽ ngừng lưu thông và giảm số lượng sẵn sàng.</p></div></div><div class="modal-footer"><a class="btn bm-btn-secondary" href="${pageContext.request.contextPath}/book-management/copies">Hủy</a><button class="btn btn-primary-custom" type="submit">Lưu thay đổi</button></div></form></div></div></div>
</c:if>
</body>
</html>
