...
<body class="d-flex flex-column">
    ...
    <div class="d-flex main-wrapper overflow-hidden">
        ...
        <main class="flex-grow-1 overflow-y-auto main-content-layout" style="background-color: var(--background);">
            ...
            <div class="container-fluid px-4 py-4">
                ...
                <!-- ─── Bộ lọc ─── -->
                <div class="card config-card mb-4 p-4">
                    <form action="${pageContext.request.contextPath}/admin/system-config" method="GET" class="d-flex gap-3 align-items-end">
                        <div class="flex-grow-1" style="max-width: 320px;">
                            <label for="groupFilter" class="form-label fw-bold small text-uppercase text-secondary"
                                   style="font-size: 11px; letter-spacing: 0.05em;">Lọc theo nhóm cấu hình</label>
                            <select class="form-select px-3" id="groupFilter" name="group" style="border: 1.5px solid var(--outline-variant); border-radius: 8px; height: 42px; font-size: 14.5px;">
                        ...
                        <button type="submit" class="btn btn-primary-custom px-4 d-flex align-items-center gap-2" style="border-radius: 8px; font-size: 13.5px; font-weight: 700; height: 42px;">
                ...
                <div class="card config-card overflow-hidden">
                    <div class="card-header bg-white py-3 px-4 border-bottom d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-0 fw-bold" style="color: var(--on-surface); font-size: 16px;">Danh sách Cấu hình</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-lms table-hover align-middle mb-0">
                            <thead>
                                <tr>
                                    <th style="width: 20%; min-width: 160px;">Tên Cấu Hình</th>
                                    <th style="width: 12%; min-width: 110px;">Nhóm</th>
                                    <th style="width: 28%; min-width: 240px;">Mô Tả</th>
                                    <th style="width: 12%; min-width: 100px;" class="text-center">Giá Trị</th>
                                    <th style="width: 13%; min-width: 130px;">Cập Nhật Bởi</th>
                                    <th style="width: 10%; min-width: 110px;">Lần Cuối</th>
                                    <th style="width: 5%; min-width: 150px;" class="text-end">Thao Tác</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach var="cfg" items="${configs}">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${cfg.configGroup == 'library'}"><span class="badge-pill badge-success"><span class="material-symbols-outlined" style="font-size: 12px; vertical-align: middle;">menu_book</span> Library</span></c:when>
                                                <c:when test="${cfg.configGroup == 'fine'}"><span class="badge-pill badge-error"><span class="material-symbols-outlined" style="font-size: 12px; vertical-align: middle;">payments</span> Fine</span></c:when>
                                                <c:when test="${cfg.configGroup == 'sepay'}"><span class="badge-pill" style="background: #e8f4fd; color: #0c63a4; border: 1px solid #b3d9f7;"><span class="material-symbols-outlined" style="font-size: 12px; vertical-align: middle;">qr_code_2</span> SePay</span></c:when>
                                                <c:when test="${cfg.configGroup == 'notification'}"><span class="badge-pill badge-neutral"><span class="material-symbols-outlined" style="font-size: 12px; vertical-align: middle;">notifications</span> Notification</span></c:when>
                                                <c:when test="${cfg.configGroup == 'system'}"><span class="badge-pill badge-neutral"><span class="material-symbols-outlined" style="font-size: 12px; vertical-align: middle;">computer</span> System</span></c:when>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex gap-1">
                                                ...
                                                <form action="${pageContext.request.contextPath}/admin/system-config" method="POST" class="m-0" onsubmit="return confirm('Bạn có chắc chắn muốn xóa cấu hình này không?');">
                                ...
                                <c:if test="${empty configs}">
                                    <tr>
                                        <td colspan="7" class="text-center py-5">
                                            ...
                                            <span style="color: var(--on-surface-variant); font-size: 13.5px; font-weight: 500;">Không có cấu hình nào trong nhóm này.</span>
                                        </td>
                                    </tr>
                                </c:if>
                            </tbody>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="${pageContext.request.contextPath}/admin/system-config" method="POST" class="modal-content" style="border-radius: 12px; border: 1px solid var(--outline-variant); overflow: hidden;">
                ...
                <div class="modal-body p-4" style="color: #262626;">
                    <div class="mb-3">
                        ...
                        <div class="form-text mt-1 text-muted" style="font-size: 11.5px;">Viết hoa, dùng dấu gạch dưới làm dấu phân cách. Key phải nằm trong whitelist.</div>
                    </div>
                    <div class="mb-3">
                        <label for="createConfigGroup" class="form-label fw-bold small text-uppercase text-secondary" style="font-size: 11px; letter-spacing: 0.05em;">Nhóm Cấu Hình <span class="text-danger">*</span></label>
                        <select class="form-select px-3" id="createConfigGroup" name="configGroup" required style="border: 1.5px solid var(--outline-variant); border-radius: 8px; height: 42px; font-size: 14.5px;">
