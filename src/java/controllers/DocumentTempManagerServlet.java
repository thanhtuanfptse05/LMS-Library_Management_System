...
public class DocumentTempManagerServlet extends HttpServlet {
    ...
    private void handleUpdate(HttpServletRequest request, HttpServletResponse response, int managerId)
        ...
        if (idParam == null || idParam.trim().isEmpty() || subject == null || subject.trim().isEmpty()) {
            response.sendRedirect(request.getContextPath() + "/manager/email-templates?error=Dữ+liệu+không+hợp+lệ");
            return;
        }
        ...
        try {
            ...
            if (updated) {
                ...
                response.sendRedirect(request.getContextPath() + "/manager/email-templates?success=Đã+cập+nhật+mẫu+email+thành+công");
            } else {
                response.sendRedirect(request.getContextPath() + "/manager/email-templates?error=Cập+nhật+thất+bại");
            }
        } catch (NumberFormatException e) {
            response.sendRedirect(request.getContextPath() + "/manager/email-templates?error=ID+không+hợp+lệ");
        }
